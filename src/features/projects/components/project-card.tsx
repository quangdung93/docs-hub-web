...
export function ProjectCard({ project }: { project: Project }) {
  ...
  return (
    <Link
      ...
      className="group border-border hover:border-brand/40 focus-visible:ring-ring/40 block rounded-xl border p-4 transition hover:shadow-sm focus-visible:ring-2 focus-visible:outline-none"
    >
      <div className="flex items-start justify-between">
        <span className="bg-brand-subtle text-brand grid size-10 place-items-center rounded-lg">
          <Folder className="size-5" aria-hidden />
        </span>
      </div>

      <h3 className="mt-3 font-medium tracking-tight">{project.name}</h3>
